...
- (UIImage*) copyImage:(UIImage *)img
{
    ...
    UIGraphicsBeginImageContextWithOptions(img.size, 1.0, 0.0);
...
void filterHue(UInt8 *pixelBuf, UInt32 offset, void *context)
{
    double H = *((double*)context);
    
    double U = cos(H*M_PI/(double)180.0);
    double W = sin(H*M_PI/(double)180.0);
  
	int r = offset;
	int g = offset+1;
	int b = offset+2;
	
	double red = (double)pixelBuf[r];
	double green = (double)pixelBuf[g];
	double blue = (double)pixelBuf[b];

	pixelBuf[r] = SAFECOLOR((.701*U+.168*W)*red + (-.587*U+.330*W)*green + (-.114*U-.497*W)*blue);
	pixelBuf[g] = SAFECOLOR((-.299*U-.328*W)*red + (.413*U+.035*W)*green + (-.114*U+.292*W)*blue);
	pixelBuf[b] = SAFECOLOR((-.3*U+1.25*W)*red + (-.588*U-1.05*W)*green + (.886*U-.203*W)*blue);

//    pixelBuf[r] = SAFECOLOR(red * 1);
//	pixelBuf[g] = SAFECOLOR(U*green + W*blue);
//	pixelBuf[b] = SAFECOLOR(-W*green + U*blue);
}
...
void filterBW(UInt8 *pixelBuf, UInt32 offset, void *context)
{
	double t = *((double*)context);
...
{	
    ...
    double temp1 = sqrt(-2.0*log((double) alpha));
    double temp2 = (double) (2.0*M_PI*beta);
	double sigma = temp1 * cos(temp2);
	double tau = temp1*sin(temp2);
...
{
	return [self applyFilter:filterHue context:&angle];
}

- (UIImage*) bwFilter:(double)levels
{
	return [self applyFilter:filterBW context:&levels];
}
